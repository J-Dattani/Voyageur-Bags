        ...
        <div class="col-12">
          ...
          <div class="card shadow mb-4">
            <div class="card-header">
              <strong class="card-title">Lists of Items</strong>
...
$(document).ready(function(){
    $('#userDataList').dataTable({
        ...
        "columns": [
          { 
            ...
            {data: 'description'},
            {data: 'percentage'},
            {data: 'price'},// Include the on_sale data
            {data: null, render: function(data, type, row) {
                // Calculate discounted price
                var price = parseInt(row.price);
                var discount = parseInt(row.percentage);
                var discountedPrice = price - (price * (discount / 100));
                return discountedPrice.toFixed(0); // Format to two decimal places
            }},
            {data: null, render: function(data, type, row) {
              if(row.on_sale)
              {
                var checkbox = '<input  type="checkbox" value="' + row.on_sale + '"  class="percentage-checkbox" checked>';
      
              }else{
                var checkbox = '<input type="checkbox" value="' + row.on_sale + '"  class="percentage-checkbox">';
      
              }
                var span = '<span>' + (row.on_sale  ? 'yes' : 'no') + '</span>';
                checkbox += span;
                return checkbox;
            }},
            {
    ...
    render: function(data, type, row) {
        var checkbox = '<input type="checkbox" class="home-items-checkbox"';
        ...
        var span = '<span>' + (row.home_items ? 'yes' : 'no') + '</span>';
...
// Add event listener to checkbox on sale
$(document).ready(function() {
    $('#userDataList').on('click', '.percentage-checkbox', function() {
        var span = $(this).next('span');
        span.text(this.checked ? 'yes' : 'no');

        // Get item ID from the first column (assuming it's visible)
        var itemId = $(this).closest('tr').find('td:eq(1)').text().trim(); // Adjust index if needed
        var onSale = this.checked ? 1 : 0; // Convert checkbox state to 1 or 0

        // Send AJAX request to update on_sale value
        $.ajax({
            type: 'POST',
            url: 'update_on_sale.php', // Update PHP script URL
            data: { id: itemId, on_sale: onSale },
...
$(document).ready(function() {
    ...
    $('#userDataList').on('change', '.percentage-checkbox', function() {
        var itemId = $(this).closest('tr').find('td:eq(0)').text(); // Get item ID from first column
        var onSale = this.checked ? 1 : 0; // Convert checkbox state to 1 or 0

        // Display SweetAlert for confirmation
        var message = this.checked ? 'Item is now on sale!' : 'Item is removed from sale.';
        var type = this.checked ? 'success' : 'success';
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'update_on_sale.php',
                    data: { id: itemId, on_sale: onSale },
                    success: function(data) {
                        // Update label immediately after successful update
                        var label = $(this).closest('tr').find('td:eq(8) span');
                        label.text(onSale ? 'yes' : 'no');
                        console.log('Updated successfully!');
                    },
                    error: function(xhr, status, error) {
    ...
    $(document).ready(function(){
    $.ajax({
        url: 'update_discounted_price.php',
        method: 'POST',
        dataType: 'json',
        success: function(response) {
            if(response.success) {
                // Update table with the new data
                $('#userDataList').DataTable().ajax.reload();
                console.log('Discounted prices updated successfully!');
            } else {
                console.error('Error updating discounted prices:', response.error);
            }
        },
        error: function(xhr, status, error) {
...
// Add event listener to checkbox on home_items
$(document).ready(function() {
    $('#userDataList').on('click', '.home-items-checkbox', function() {
        var span = $(this).next('span');
        span.text(this.checked ? 'yes' : 'no');

        // Get item ID from the first column (assuming it's visible)
        var itemId = $(this).closest('tr').find('td:eq(1)').text().trim(); // Adjust index if needed
        var homeitems = this.checked ? 1 : 0; // Convert checkbox state to 1 or 0

        // Send AJAX request to update on_sale value
        $.ajax({
            type: 'POST',
            url: 'update_on_home_items.php', // Update PHP script URL
            data: { id: itemId, home_items: homeitems },
            success: function(data) {
                console.log('Updated successfully!');
            },
            error: function(xhr, status, error) {
...
$(document).ready(function() {
    // Add event listener to checkbox
    $('#userDataList').on('change', '.home-items-checkbox', function() {
        var checkbox = this;
        var itemId = $(checkbox).closest('tr').find('td:eq(1)').text().trim(); // Adjust index if needed
        var homeItems = checkbox.checked ? 1 : 0; // Convert checkbox state to 1 or 0

        // Display SweetAlert for confirmation
        var message = checkbox.checked ? 'Item added to Home page products!' : 'Item removed from Home page products.';
        var type = checkbox.checked ? 'success' : 'success';
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Send AJAX request to update home_items value
                $.ajax({
                    type: 'POST',
                    url: 'update_on_home_items.php',
                    data: { id: itemId, home_items: homeItems },
                    success: function(data) {
                        // Update label immediately after successful update
                        var label = $(checkbox).closest('tr').find('td:eq(2) span'); // Assuming index 2 is where the label is
                        label.text(homeItems ? 'yes' : 'no');
                        console.log('Updated successfully!');
                    },
                    error: function(xhr, status, error) {
                        console.error('Error updating status:', error);
