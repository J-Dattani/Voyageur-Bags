...
if (isset($_GET['s_id']) && !empty($_GET['s_id'])) {
    ...
    // Prepare and execute the query to fetch store details
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['error'] = "No record found for the provided ID.";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['image']['tmp_name']) && !empty($_FILES['image']['tmp_name'])) {
        ...
        if (!in_array($fileType, $allowTypes)) {
            ...
            header("location: ./stores_edit.php?s_id=$s_id");
            exit();
        }
        ...
        $update_query = "UPDATE stores SET location=?, phone=?, image=? WHERE id=?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("sssi", $location, $phone, $fileName, $id); // Updated parameter binding for image

    } else {
        // Update query without image
        $update_query = "UPDATE stores SET location=?, phone=? WHERE id=?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("ssi", $location, $phone, $id);
    }
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['error'] = "Error updating store details: " . $conn->error;
        header("location: ./stores_edit.php?s_id=$s_id");
        exit();
    }
}
            ...
            <div class="card-body">
              <div class="row">
                ...
                <div class="col-md-6">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                          ...
                          <input class="form-control form-control-lg" id="phone" name="phone" type="text" value="<?php echo ($row['phone']); ?>" required>
                ...
                <label for="image">Add Image</label>
                  ...
                  <a type="submit" href="stores_list.php" class='btn btn-danger'>Dismiss</a>
